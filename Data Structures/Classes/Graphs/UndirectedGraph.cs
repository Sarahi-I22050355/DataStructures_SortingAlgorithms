...
namespace DataStructures_SortingAlgorithms.Data_Structures.Graphs
{
    internal class UndirectedGraph<T>
    {
        ...
        public Dictionary<T, int> CalculateBFSLevels(T startVertex)
        {
            ...
            Console.WriteLine("Levels BFS: " + string.Join(", ", levels.Select(pair => $"{pair.Key}:{pair.Value}")));
            return levels;
        }
    }
}